...
const arrayToCSVString = array => {
  // your code here
  const csvString = array.toString();
  return csvString;
};
...
const reverseWordsInArray = strings => {
  ...
  const output = whitespace
    .split(' ')
    .map(
      // split into words and iterate via map
      s =>
        s
          .split('')
          .reverse()
          .join(''), // split individual words into characters and then reverse the array of character and join it back
    )
...
const removeNthElement2 = (index, array) => {
  // your code here
  n = index;
  const createdArray = array.slice();
...
const elementsStartingWithAVowel = strings => {
  // your code here
  const startWithVowel = strings.filter(str => /^[aeiou]/i.test(str));
